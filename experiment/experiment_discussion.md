...
# Experiment Update 1: Inner Class Support & "File Not Annotated" Category

## Changes Made

### 1. Inner Class Resolution in JRT Mode (Fixed)

The biggest source of `NOT_ANALYZED` methods was inner classes not being loaded. This has been fixed.

**Before:** `detectJdkClasses()` in `Main.java` extracted only the top-level class name from the file path:

```
HashMap.java → java.util.HashMap (1 class)
```

All inner classes (`HashMap$Node`, `HashMap$KeySet`, `HashMap$EntrySet`, etc.) were invisible to the analysis.

**After:** `detectJdkClasses()` now scans the JRT filesystem (`jrt:/modules/<module>/<package>/`) for all `.class` files matching `TopLevelClass$*.class` and includes their FQCNs:

```
HashMap.java → java.util.HashMap, java.util.HashMap$Node, java.util.HashMap$TreeNode,
               java.util.HashMap$KeySet, java.util.HashMap$Values, java.util.HashMap$EntrySet,
               java.util.HashMap$EntryIterator, ... (all inner classes)
```

**Implementation:** After resolving the top-level FQCN from the file path, the code opens the JRT filesystem and lists the package directory for files matching the `<ClassName>$*.class` pattern:
...
Files.list(moduleDir)
    ...
    .forEach(name -> {
        String innerFqcn = (packageDir + name.replace(".class", "")).replace('/', '.');
        classNames.add(innerFqcn);
    });
```

**Verified on ArrayList.java:** Before: 72 top-level methods analyzed. After: 143 methods analyzed (71 from inner classes `Itr`, `ListItr`, `SubList`, `ArrayListSpliterator`).

### 2. "File Not Annotated" Category (New)

The original experiment categorized all unannotated methods that the tool reports as `SIDE_EFFECT_FREE` under a single label: **Annotation Deficit**. This conflated two very different situations:

1. **Genuine annotation deficit** — The file *does* have `@Pure`/`@SideEffectFree` annotations on other methods, but this particular method was not annotated despite being side-effect-free. This suggests the Checker Framework team missed it, or intentionally left it unannotated.

2. **Entirely unannotated file** — The file has *zero* annotations anywhere. The Checker Framework team simply hasn't annotated this file at all. Labeling individual methods as "annotation deficit" is misleading because the file was never in scope for annotation.

**Fix:** The `categorize()` function in `run_experiment.py` now checks whether the file containing the method has any annotations at all:

- **Annotation Deficit**: Tool says `SIDE_EFFECT_FREE`, method is not annotated, but the file *does* contain other `@Pure`/`@SideEffectFree` annotations. These are meaningful — they represent methods the Checker Framework could potentially annotate.
- **File Not Annotated**: Tool says `SIDE_EFFECT_FREE`, but the *entire file* has no annotations. These are not meaningful comparisons and should be excluded from precision/recall calculations.

### Impact on Results

These two changes affect the experiment results in complementary ways:

- **Inner class support** reduces the `Not Analyzed` count significantly by bringing inner class methods (iterators, entry sets, sub-lists, etc.) into analysis scope.
- **File Not Annotated** separates noise from the `Annotation Deficit` count, making the remaining annotation deficits more meaningful for evaluation.

To regenerate results with both changes, run:
