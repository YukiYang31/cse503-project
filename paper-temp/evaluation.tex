...
% ============================================================
\textcolor{red}{This entire section is outdated because with the JDK cache, we need a new way to measure runtime of JDK methods (some of them are already cached)}
...
The Pearson correlation between graph edge count and analysis time is $r = 0.35$, confirming that graph size is a moderate predictor of cost. \textcolor{red}{For Mike: It is still the case that interprocedural calls takes up the most amount of time. But current time data are from an old study.}
...
We compare the tool's cached verdicts for \texttt{java.util} methods against the ground truth.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{File} & \textbf{Annotated} & \textbf{Match} & \textbf{Mismatch} & \textbf{Agreement} \\
\midrule
...
\subsection{RQ3: Downstream analysis}
We will demonstrate the utility of our tool by integrating its output into two of the three widely used software analysis tools. 
\begin{itemize}
