...
We demonstrate the analysis on a two-method example that captures the key algorithmic steps: intra-procedural dataflow, the constructor exception rule, inter-procedural summary instantiation, and side-effect checking.
...
Our analysis first uses Algorithm \ref{alg:ComputeBottomUpOrder} to obtain a call graph and the corresponding batch order. The call graph places the \texttt{BankAccount} constructor before \texttt{Wallet.swapAccount()}, so the constructor's summary is cached before the caller is analyzed.

%% ────────────────────────────────────────────────────
\subsubsection{Analysis of \texttt{BankAccount(Object owner, int amount)} (callee)}
The constructor is fully intra-procedural -- it contains no method calls. The analysis processes it via forward dataflow (Figure \ref{alg:intraprocedural}), applying transfer functions to each statement:

\begin{enumerate}
\item \textbf{Identity} (\hyperref[agl4a]{rule a) of 3.3.1}): \texttt{this} $\to P_0$, \texttt{owner} $\to P_1$. The primitive parameter \texttt{amount} is not tracked in the points-to graph.

\item \textbf{Field store} (\hyperref[agl4e]{rule e) of 3.3.1}): Writing \texttt{this.owner = owner} with $L(\texttt{this}) = \{P_0\}$ and $L(\texttt{owner}) = \{P_1\}$ adds an inside edge $P_0 \xrightarrow{\texttt{owner}} P_1$ to $I$ and records $(P_0, \texttt{owner})$ in $W$.

\item \textbf{Field store} (\hyperref[agl4e]{rule e) of 3.3.1}): Writing \texttt{this.amount = amount} --- since \texttt{amount} is a primitive (\texttt{int}), no inside edge is created (there is no reference target). However, the mutation $(P_0, \texttt{amount})$ is still recorded in $W$.
\end{enumerate}

\noindent
\Cref{fig:ctor-exit} shows the exit graph. Both nodes are ParameterNodes (blue ellipses). The inside edge $P_0 \xrightarrow{\texttt{owner}} P_1$ records the reference field store \texttt{this.owner = owner}. The primitive store \texttt{this.amount = amount} adds $(P_0, \texttt{amount})$ to $W$ but no edge (primitives have no pointer targets). Although $P_0$ is mutated and $P_0 \in A$ (prestate), the constructor exception rule skips $P_0$: initializing \texttt{this} in a constructor is allowed.
  The final verdict indicates that the method is \textsc{side\_effect\_free}.
...
%% ────────────────────────────────────────────────────
\subsubsection{Analysis of \texttt{Wallet.swapAccount(Object newOwner)} (caller)}

\begin{enumerate}
\item \textbf{Identity} (\hyperref[agl4a]{rule a) of 3.3.1}): \texttt{this} $\to P_0$, \texttt{newOwner} $\to P_1$.

\item \textbf{Allocation + Constructor Invoke} (\hyperref[agl4b]{rule b) of 3.3.1}, \hyperref[agl4i]{rule i) of 3.3.1}): 
  ...
  The cached constructor summary is instantiated into the caller's graph via Algorithm \ref{alg:graph-combination}:
...
\noindent
\Cref{fig:swap-exit} shows the final exit graph.
$P_0$ (double red border) is a mutated prestate node: the inside edge $P_0 \xrightarrow{\texttt{account}} I_0$ records the field store \texttt{this.account = temp}, and $(P_0, \texttt{account}) \in W$. $I_0$ (green box) is the freshly allocated \texttt{BankAccount}; the edge $I_0 \xrightarrow{\texttt{owner}} P_1$ was introduced by instantiating the constructor summary. Since $P_0 \in A$ and $(P_0, \texttt{account}) \in W$, the verdict is \textsc{side\_effecting}.
