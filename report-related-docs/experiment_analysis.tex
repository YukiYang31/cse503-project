...
The implementation described in this report now uses a complete-call-graph pipeline with BFS JDK discovery, bottom-up summary composition, and a disk-backed JDK summary cache. Any quantitative results presented in this section therefore need to be interpreted as results for the \emph{current} pipeline only if they were regenerated from the current experiment scripts and CSV artifacts. Older text that referred to the retired on-demand inter-file analysis model has been removed.
...
Each file is analyzed independently for the experiment harness, but within each run the tool first discovers reachable JDK classes by bounded BFS, constructs a merged call graph, and then analyzes methods in bottom-up call-graph order.
Cross-file JDK callees are therefore incorporated through the discovered call graph and summary cache rather than via the retired on-demand inter-file analysis path.
...
Only 6 methods (0.1\%) exceed one second, concentrated in especially heavy inter-procedural cases (e.g., \texttt{Formatter.format} at 2.7s) or complex static initialization (e.g., \texttt{Tripwire.<clinit>} at 3.1s).
